...
LV_SS <- read.csv("/Users/chirp/DRIVE/PHD/METHODS/Photo_ID/catalogue_files/Original/LVMaster_Jan13.csv", colClasses = ("Date" = "character") )
...
LV_SS$QRATE = as.numeric(LV_SS$QRATE)
   ...
   # ALL M/FJ categories-----
    ...
    LV_SS = LV_SS%>%group_by(ID)%>%fill(Sex, .direction = "downup")%>%mutate(Sex = ifelse(is.na(Sex), "UNK", Sex))
    ...
    Total_ID = LV_SS%>%group_by(ID, Sex)%>%summarise(N = n())
            ...
            write_csv(Id_Year2, "/Users/chirp/DRIVE/PHD/METHODS/Photo_ID/catalogue_files/ID_SEX_MASTER.csv")
            ...
            write_csv(LV_SS, "/Users/chirp/DRIVE/PHD/METHODS/Photo_ID/catalogue_files/LV_SS_MASTER_Jan13.csv")
            
            
# # #export clean version for socprog-------
      #still need to open in excel and format date column there for some reason...?
       SOCPROGNBW_2020 = select(LV_SS,
                                c("QRATE","Date","Location", "Latitude", "Longitude",
                                  "side", "Reliable","Sex", "ID"))
            write.csv(SOCPROGNBW_2020, "~/Documents/MATLAB/SOCPROG/Uncompiled/catalogue/SOCPROGNBW_2020.csv", row.names = FALSE)
# #
# #
# # #supplementary data for sex-----
 SOCPROG_SUPDATA = LV_SS%>% select(ID, Sex, side) %>%
              group_by(ID, side, Sex)%>% summarise(count=n())
 
            write.csv(SOCPROG_SUPDATA, "~/Documents/MATLAB/SOCPROG/Uncompiled/catalogue/SOCPROG_SUPDATA.csv", row.names = FALSE)
