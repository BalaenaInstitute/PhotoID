...
#recode SIDE------
LV_SS$keyword = LV_SS$Keyword.export
...
LV_SS=LV_SS%>%mutate(Location = ifelse(Location == "UNK" & Latitude >= 44, 
                                       "Shortland", ifelse(Location == "UNK" & Latitude <= 44, "Gully", Location)))
...
write_csv(Id_Year2, here("catalogue_files/ID_SEX_MASTER.csv"))
...
write.csv(SOCPROG_SUPDATA21, here("socprog/SOCPROG_SUPDATA2021.csv"), row.names = FALSE)
